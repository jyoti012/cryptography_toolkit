...
public class PasswordBasedDecryption {
	public static void main(String[] args) {
		try {
			String[] passwords = { "Cork", "Dublin", "Wexford", "Kerry", "Kildare", "Kilkenny", "Mayo", "Galway", "Waterford",
					"Meath", "Limerick" };
			...
			for (String p : passwords) {
				...
				for (salt = 0; salt <= 0xffff; salt++) { // bruteforce all the possibilities of 2 byte salt 0x0000 to 0xffff
					byte[] b = { (byte) ((salt >> 8) & 0x000000ff), (byte) (salt & 0x000000ff) };// seperate the salt to byte
																																												// array
